...
namespace Summit_Interface
{
    class SummitProgram
    {
        ...
        static void Main(string[] args)
        {
            ...
            while (true)
            {
                Console.WriteLine("Do you want to run in MyRC+S testing mode (dont actually connect to the RC+S)? [y/n]: ");
                input = Console.ReadKey(true);

                if (input.KeyChar.ToString() == "y")
                {
                    myRCSTesting = true;
                    Console.WriteLine("Will not attempt to connect to the INS");
                    break;
                }
                else if (input.KeyChar.ToString() == "n")
                {
                    Console.WriteLine("Will attempt to connect to the INS");
                    myRCSTesting = false;
                    break;
                }
            ...
            if (!myRCSTesting)
            {
                ...
                sharedResources.testMyRCPS = false;
                ...
                //start MyRC+S connection thread
            ...
            {
                //run MyRC+S connection thread
                ThreadResources sharedResources = new ThreadResources();
                sharedResources.TDbuffer = m_TDBuffer;
                sharedResources.savingBuffer = m_dataSavingBuffer;
                sharedResources.summit = m_summit;
                sharedResources.summitManager = null;
                sharedResources.saveDataFileName = m_dataFileName;
                sharedResources.samplingRate = TdSampleRates.Disabled;
                m_timingLogFile = new ThreadsafeFileStream("Testing-Timing.txt");
                sharedResources.timingLogFile = m_timingLogFile;
                sharedResources.parameters = parameters;
                sharedResources.testMyRCPS = true;
                StreamingThread myRCSThread = new StreamingThread(ThreadType.myRCS);
                myRCSThread.StartThread(ref sharedResources);

                //wait until user pushes "q" button to quit
                ConsoleKeyInfo thekey = new ConsoleKeyInfo();
                Console.WriteLine("Running test program...press 'q' to exit");
                while (thekey.KeyChar.ToString() != "q")
                {
                    thekey = Console.ReadKey(true);
                }
